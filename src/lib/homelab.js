...
export function generateNodes() {
  ...
  let nx = 50;
  const netOrder = ['isp-modem', 'ucg-max', 'usw-flex', 'tplink-sg108e', 'u7-pro', 'tv'];
  for (const key of netOrder) {
    ...
    nodes.push({
      ...
      position: { x: nx, y: Y_NET },
      dimensions: { width: 120, height: 50 },
      layer: 1, parent: null, data: entry, category: 'network',
    });
    nx += 150;
  }
  ...
  const svcConfigs = {
    ...
    pi3b: [
      { keys: ['squeezelite', 'wyoming-satellite', 'openWakeWord'], cats: ['package', 'container', 'container'], y: Y_SVC1 },
    ],
  };
  ...
  // pi5 column
  {
    ...
    colDefs.push({ hwKey: 'pi5', width: Math.max(maxRowW, HW_WIDTH), svcW, svcGap, rows });
  }
  ...
  // fbox column
  {
    ...
    colDefs.push({ hwKey: 'fbox', width: colW, vmW, vmGap, lxcW, lxcGap, nVms, nLxcs });
  }
  ...
  // pi3b column
  {
    ...
    colDefs.push({ hwKey: 'pi3b', width: Math.max(w, HW_WIDTH), svcW, svcGap });
  }

  // standalone hardware
  for (const key of ['darkforce', 'feebook', 'guestbook']) {
    colDefs.push({ hwKey: key, width: HW_WIDTH });
  }
      ...
      for (const row of col.rows) {
        ...
        for (let i = 0; i < row.keys.length; i++) {
          ...
          nodes.push({
            ...
            layer: 3, parent: 'hw-pi5', data: data || {},
      ...
      if (vmKeys.length > 0) {
        ...
        for (let i = 0; i < vmKeys.length; i++) {
          ...
          nodes.push({
            ...
            layer: 3, parent: 'hw-fbox', data: vms[key], category: 'vm',
          });
        }
      }
      ...
      for (let r = 0; r < nRows; r++) {
        ...
        for (let i = 0; i < rowKeys.length; i++) {
          ...
          nodes.push({
            ...
            layer: 3, parent: 'hw-fbox', data: lxcs[key], category: 'lxc',
          });
        }
      }
    }
    ...
    if (col.hwKey === 'pi3b') {
      ...
      for (let i = 0; i < keys.length; i++) {
        ...
        nodes.push({
          ...
          dimensions: { width: col.svcW, height: 50 },
          layer: 3, parent: 'hw-pi3b', data: data || {},
          category: cat === 'package' ? 'package' : 'container',
        });
      }
    }
...
export function getNodeDetails(nodeId) {
  ...
  if ((node.category === 'vm' || node.category === 'lxc' || node.category === 'package' || node.category === 'container') && node.data) {
